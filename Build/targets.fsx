...
let toolPackages =
    let xml = "./Build/dotnet-cli.csproj" |> Path.getFullName |> XDocument.Load
...
_Target "BuildRelease" (fun _ ->
    try
        ...
        "./altcode.test/altcode.test.sln" |> dotnetBuildRelease
    with x ->
        printfn "%A" x
        reraise ())
...
_Target "BuildDebug" (fun _ ->
    DotNet.restore (fun o -> o.WithCommon(withWorkingDirectoryVM ".")) "./altcode.test/altcode.test.sln"
    "./altcode.test/altcode.test.sln" |> dotnetBuildDebug)
...
_Target "PrepareDotNetBuild" (fun _ ->
    ...
    [ ("./_Publish/expecto/altcode.test.expecto.nuspec", "AltCode.Test.Expecto (Expecto helper)")
    ...
    |> List.iter (fun (path, caption) ->
        ...
        [ "authors" ]
        |> List.iter (fun tag ->
            let title = dotnetNupkg.Descendants(x tag) |> Seq.head
        ...
        |> Seq.iter (fun (a, b, l) ->
            ...
            l |> Seq.iter (fun (n, v) -> node.SetAttributeValue(XName.Get(n, ""), v))
...
"Preparation" ==> "BuildRelease"

"BuildRelease" ==> "BuildDebug" ==> "Compilation"

"BuildRelease" ==> "Lint" ==> "Analysis"

"Compilation" ==> "Analysis"

"Compilation" ?=> "Packaging"

"Compilation" ==> "PrepareDotNetBuild" ==> "Packaging"

"Compilation" ==> "PrepareReadMe" ==> "Packaging" ==> "Deployment"
